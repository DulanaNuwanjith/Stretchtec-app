                        ...
                        <div class="p-4 text-gray-900 dark:text-gray-100">
                            ...
                            <div id="TapeTDRecordsScroll"
                                ...
                                <table class="table-fixed w-full text-sm divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    <tbody id="TapeTDRecords" class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    @forelse($technicalCardTapes as $technicalCardTape)
                                        <tr>
                                            <!-- Reference -->
                                            <td class="text-center px-4 py-3">
                                                <div class="font-bold">{{ $technicalCardTape->reference_number ?? '-' }}</div>
                                            </td>

                                            <!-- Item Details -->
                                            <td class="px-4 py-3 text-xs text-left break-words">
                                                <div>Created Date: {{ $technicalCardTape->created_at?->format('Y-m-d') ?? '-' }}</div>
                                                <div>Type: {{ $technicalCardTape->type ?? '-' }}</div>
                                                <div>Size: {{ $technicalCardTape->size ?? '-' }}</div>
                                                <div>Color: {{ $technicalCardTape->color ?? '-' }}</div>
                                                <div>Machine: {{ $technicalCardTape->machine ?? '-' }}</div>
                                            </td>

                                            <!-- Technical Details -->
                                            <td class="px-4 py-3 text-xs text-left break-words">
                                                <div>Weft Yarn: {{ $technicalCardTape->weft_yarn ?? '-' }}</div>
                                                <div>Warp Yarn: {{ $technicalCardTape->warp_yarn ?? '-' }}</div>
                                                <div>Wheel Up: {{ $technicalCardTape->wheel_up ?? '-' }}</div>
                                                <div>Wheel Down: {{ $technicalCardTape->wheel_down ?? '-' }}</div>
                                                <div>Reed: {{ $technicalCardTape->reed ?? '-' }}</div>
                                                <div>Rubber Type: {{ $technicalCardTape->rubber_type ?? '-' }}</div>
                                            </td>

                                            <!-- Quality Details -->
                                            <td class="px-4 py-3 text-xs text-left break-words">
                                                <div>Special Remarks: {{ $technicalCardTape->special_remarks ?? '-' }}</div>
                                            </td>
                                            ...
                                            <td class="px-4 py-3 text-center">
                                                <div class="flex gap-2 justify-center items-center">
                                                    ...
                                                    @if(!$technicalCardTape->url)
                                                        <form action="{{ route('technicalCards.storeImage', $technicalCardTape->id) }}" method="POST" enctype="multipart/form-data" class="inline-block">
                                                            @csrf
                                                    ...
                                                    <!-- Delete Button -->
                                                    <form action="{{ route('technicalCards.delete', $technicalCardTape->id) }}" method="POST"
                            ...
                            <div id="addTapeTDModal"
                                ...
                                <div class="w-full max-w-[700px] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-4 transform transition-all scale-95 max-h-[calc(100vh-10rem)] overflow-y-auto"
                                    ...
                                    <div class="max-w-[600px] mx-auto p-8">
                                        ...
                                        <form action="{{route('tapeTD.create')}}" method="POST" enctype="multipart/form-data">
                                            @csrf
                                            <div class="space-y-4">
