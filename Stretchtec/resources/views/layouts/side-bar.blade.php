...
<aside x-data="{ collapsed: window.__initialCollapsed, initialized: false }" x-init="initialized = true"
            ...
            @if (in_array($role, ['ADMIN', 'SUPERADMIN']))
                <li>
                    <a href="{{ route('production-inquery-details.index') }}"
                       class="flex items-center px-4 py-2 rounded hover:bg-gray-200
                      {{ request()->routeIs('production-inquery-details.*', 'production-order-preparation.*', 'mail-booking.*', 'mail-booking-approval.*') ? 'bg-gray-200' : '' }}">
